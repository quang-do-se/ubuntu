...
** Update/upgrade system
   ...
   - Update grub to always display grub menu in /etc/default/grub
...
** Handle broken Linux Kernel
   ...
   #+begin_src sh
     sudo apt update
     sudo apt install --reinstall linux-image-<kernel-version>
     sudo apt install --reinstall linux-modules-<kernel-version>
...
** Scan system
   ...
   #+begin_src sh
     sudo clamscan -r --bell -i --remove=yes /home/

     sudo chkrootkit

     # Display TCP and UDP listening port with attached process
     sudo netstat -tulpn
   #+end_src
